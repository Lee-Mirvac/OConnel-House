...
function Map(name) {
  ...
  this.mapDataJSON;
  this.textDataJSON;
  this.addMarker = function (icon, cordX, cordY,textcordX, textcordY,title, alignment,name) {
    var markerName;
    ...
    this.textMarker = function (icon, cordX, cordY,textcordX,textcordY,title,alignment, name) {
    var markerName;
  ...
  this.importJSON = function (mapDataJSON) {
    ...
    for (importedMarkerName in this.mapDataJSON) {
      // add new marker
       ...
       this.mapDataJSON[importedMarkerName]
      this.addMarker(
        ...
        this.mapDataJSON[importedMarkerName].cordY,
         this.mapDataJSON[importedMarkerName].textcordX,
            this.mapDataJSON[importedMarkerName].textcordY,
      this.mapDataJSON[importedMarkerName].title,
       this.mapDataJSON[importedMarkerName].alignment,
      importedMarkerName
      );
      ...
      if (
        ...
        this.mapDataJSON[importedMarkerName]["modal"].title &&
         this.mapDataJSON[importedMarkerName]["modal"].p &&
         this.mapDataJSON[importedMarkerName]["modal"].address &&
...
function MapMarker(name, icon, cordX, cordY,textcordX,textcordY,title,alignment, map) {
  this.map = map;
  this.name = name;
  this.icon = icon;
  this.cordX = cordX;
    this.cordY = cordY;
  this.textcordX=textcordX,
  this.textcordY= textcordY,
  this.title= title,
this.alignment=alignment,
  this.markerContainer = this.map.container; // jquery map marker container object
  this.modal;

  this.addModal = function (modalTitle,modalP,modalAddress, modalContent) {

    this.modal = new MarkerModal(modalTitle,modalP,modalAddress, modalContent, this);
  };
  this.textModal = function (modalTitle,modalP,modalAddress, modalContent) {
 ...
 this.generateMarker = function () {
    output =
      ...
      '" alt="' +
      this.location +
      '"><span class="marker_txt '+this.alignment+'" style="">'+this.title+'</span></div>';
  ...
  this.show = function () {
   //this.markerContainer.append(this.textMarker());
  this.markerContainer.append(this.generateMarker());
    
  };
} // MapMarker class end

function MarkerModal(modalTitle,modalP,modalAddress, content, linkedMapMarker) {
  this.title = modalTitle;
  this.p= modalP;
  this.address= modalAddress,
  ...
  this.generateModal = function () {
    output =
      '<div  class="KBmap__markerContent"><div class="KBmap__markerClose"><i class="fa fa-times" aria-hidden="true"></i></div><div class="KBmap__marker_main">' +
      '<h2>' + this.title + '</h2>'
      "</div>";
 output +=
      '<h6>' + this.p + "</h6>";
...
jQuery(document).ready(function () {
  ...
  jQuery("body").on("click", ".KBmap__marker img", function () {
    let image = document.querySelector('.KBmap__markerClose');
    if(image){
      image.click();
    }
...
function createKBmap(name, mapsrc, mapDataJSON) {
  ...
  var output =
    '<div class="KBmap__mapContainer"><div class="KBmap__mapHolder"><img id="imgID" src="' +
    mapImg +
    '" alt="mapa"></div></div>';
